...
* `<style>` εισαγωγή κανόνων μορφοποίησης (CSS)
...
```
(Την πλήρη λύση θα βρείτε στο αρχείο `question.html`)
...
### Κανόνες CSS
Ένας κανόνας CSS ορίζει την εμφάνιση των στοιχείων HTML της ιστοσελίδας.

Για παράδειγμα αν επιθυμούμε τα στοιχεία `<h1>` και `<p>` να περιέχουν κείμενο χρώματος `blue`  με εσωτερικό περιθώριο `1em` o σχετικός κανόνας CSS θα έχει την εξής μορφή:
```css
h1, p {
    color: blue;
    padding: 1em;
...
```
Στο παράδειγμα αυτό οι `h1, p` λέγονται **επιλογείς**, οι `color`, `padding` είναι οι **ιδιότητες** και `blue`, `1em` είναι αντίστοιχα οι τιμές των ιδιοτήτων αυτών.
...
Το Heroku είναι μια πλατφόρμα ως υπηρεσία (Platform as a Service, PaaS) που επιτρέπει στους προγραμματιστές να δημιουργούν, να εκτελούν και να χρησιμοποιούν εφαρμογές πλήρως στο cloud.
